...
function Carplate() {
  ...
  const Change = async () => {
    ...
    if (name.length != 0) {
      ...
      fetch('http://' + ip_add['ip'] + '/api/ChangeUsercarplate/' + val['id']+'/'+name, {
        method: 'GET'
      })
        .then((response) => response.json())
        .then((responseJson) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.formContainer}>
        <Text style={styles.title}>Change Plate</Text>
        <View style={styles.card}>
          <View style={styles.inputContainer}>
            <Text style={styles.label}>Change Plate</Text>
            <TextInput
              style={styles.input}
              value={name}
              onChangeText={setName}
              placeholder={Detailsofuser.carplate}
              placeholderTextColor="#999"
            />
          </View>

          <TouchableOpacity style={styles.button} onPress={() => Change()}>
            <Text style={styles.buttonText}>Change Plate</Text>
          </TouchableOpacity>
        </View>
      </View>
    </View>
