...
function Clientlist(props) {
    ...
    const deleterequest = async (id) => {
        ...
        fetch('http://' + ip_add['ip'] + '/api/Carpoolre/' + id + '/', requestOptions).then((response) => {
        }).then((result) => {
    ...
    const Getdetails = async () => {
        ...
        fetch('http://' + ip_add['ip'] + '/api/Carpoolre/', {
            method: 'GET'
        })
            .then((response) => response.json())
            .then((responseJson) => {
                responseJson.map(item => {
                    ...
                    if (item["client_id"] == val['id'] && item['status'] == "Fail" && (item['date'] == nextDayFormatted || item['date'] == current)) {
                    ...
                    else if (item["client_id"] == val['id'] && item['status'] == 'Pending' && (item['date'] == nextDayFormatted || item['date'] == current)) {
                        i++;
                        fetchPlaceName(item["lat"], item["long"])
                        fetchPlaceNameto(item["des_lat"], item["des_long"])
                        ...
                        console.log(item)
                        console.log(nextDayFormatted,item['date'])
                        console.log(item['date'],current)
        ...
        <View style={[styles.container, { backgroundColor: "#2921c4" }]}>
            ...
            <CustomCard elevated={true} style={{ backgroundColor: "#fff", marginHorizontal: 24, marginTop: -40, padding: 10, borderRadius: 10, flexDirection: "row", justifyContent: "space-between" }}>
                <View style={{ alignItems: "center", marginLeft: 40 }}>
                    <Text onPress={() => navigation.navigate("Paymentscreen")} style={{ fontWeight: "bold", marginBottom: 10 }}>Balance</Text>
