...
function PrivateRoom() {
    ...
    const Getdetails = async () => {
        ...
        fetch('http://' + ip_add['ip'] + '/api/ClientHistory/', {
            method: 'GET'
        })
            .then((response) => response.json())
            .then((responseJson) => {
                    ...
                    if (item["client_id"] == val['id']) {
                        ...
                        Getdetails21(item['assien_driver']).then(responseJson => {
                            ...
                            dataof.push({
                                ...
                                carmodel:responseJson.carmodel,
                                carplate:responseJson.carplate,
                                time: item['time'],
                            ...
                            renderItem={({ item }) => (
                                <View style={styles.card}>
                                    <View style={styles.item}>
                                        ...
                                        <Text style={styles.userName}><B>Driver Name: </B>{item.name}</Text>
                                        <Text style={styles.userName}><B>Car Number: </B>{item.carplate}</Text>
                                        <Text style={styles.userName}><B>Car Model: </B>{item.carmodel}</Text>
