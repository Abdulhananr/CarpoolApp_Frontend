...
function Password(props) {
  ...
  const Change = async () => {
    ...
    if (name.length != 0 && email.length != 0 && password.length != 0) {
      if (Detailsofuser.password == name) {
        if (email == password && password.length > 7) {
          fetch('http://' + ip_add['ip'] + '/api/ChangeUserPassword/' + val['id'] + '/' + password, {
            method: 'GET'
          })
            .then((response) => response.json())
            .then((responseJson) => {
              storeData({ id:val['id'], email: Detailsofuser.email, password: password,username:Detailsofuser.username,image:Detailsofuser.Profile })
